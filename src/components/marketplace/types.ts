...
export interface Listing { listingId:string; title:string; description:string; category:Category; brand:string; model:string; condition:Condition; attributes:Record<string,string>; includedAccessories:string[]; visiblyMissingAccessories:string[]; evidence:Evidence[]; assumptions:Assumption[]; price:Money; status:"active"|"sold"; canDelete:boolean; seller:{id:string;displayName:string;fictional:true}; photoIds:string[]; publishedAt:string }
export interface MediaReference { id:string; mediaType:"image"; mimeType:"image/jpeg"|"image/png"|"image/webp"; alt:string; width:number;height:number }
