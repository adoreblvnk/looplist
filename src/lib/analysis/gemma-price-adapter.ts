...
export const GEMMA_PRICING_INSTRUCTIONS = `Return only a JSON object with exactly these six top-level keys and no markdown: recommendedAtomicAmount (positive integer string), minimumAtomicAmount (positive integer string), maximumAtomicAmount (positive integer string), comparables (array of objects containing comparableId string, similarityScore number from 0 to 1, and similarityReason string), strongestComparableIds (array of comparableId strings), and rationale (string). Use only the authoritative listing projection and sold-comparable projection supplied below. Do not use tools, network access, external comparables, or unstated facts. Select only supplied comparable IDs. Explain each selected comparable's fit by product/model, condition, attributes, and included accessories. Recommend exact atomic USDC amounts for the recommendation, minimum, and maximum, with minimum <= recommendation <= maximum. Explain the strongest matches and pricing range. Do not include or infer seller secrets, wallet addresses, or personal data.`;
